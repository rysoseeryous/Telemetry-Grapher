...
from .ip import Import_Parameters
from .g import Group
...
#                read_func = pd.read_csv
            ...
            try:
                ...
                print(dtf, end-start)
                if any(ts == pd.NaT for ts in data.index): raise ValueError('Timestamps could not be parsed from given index column. Check import parameters.')
    ...
    def import_group(self):
        ...
        if self.verify_import_parameters(source_files):#result == QDialog.Accepted:
    ...
    def verify_import_parameters(self, source_files):
        DM = self.parent
        AB = DM.parent
        counter = 1
        ...
        DM.feedback('Verify import parameters... ', mode='append')
        DM.messageLog.repaint()
        self.importparameters = Import_Parameters(self, source_files)
        self.importparameters.setModal(True)
        return self.importparameters.exec()
