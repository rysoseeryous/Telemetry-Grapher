...
from .af import Axes_Frame
from .cf import Control_Frame
from .sf import Series_Frame
from .dm import Data_Manager
from .fs import Figure_Settings
...
class Application_Base(QMainWindow):
    ...
    def __init__(self, logger, groups={}):
        ...
        f = QFile('rc/dark.qss')
        if not f.exists():
            self.logger().error('Unable to load dark stylesheet, file not found')
            self.dark_qss = ''
        else:
        ...
        if not f.exists():
            self.logger().error('Unable to load light stylesheet, file not found')
            self.light_qss = ''
        else:
            f.open(QFile.ReadOnly | QFile.Text)
            ts = QTextStream(f)
...
#                pkl.dump(AF.fig, f)
    ...
    def interpret_unit(self, unit):
        ...
        - Check if unit can be associated with a unit type (see self.get_unit_type).
